...
!BL
	...
	END INTERFACE
	INTERFACE
		SUBROUTINE odeint(ystart,x1,x2,eps,h1,hmin,derivs,jacobn)
		USE nrtype
...
!BL
	...
	END INTERFACE
	INTERFACE
		SUBROUTINE stiff(y,dydx,x,htry,eps,yscal,hdid,hnext,derivs,jacobn)
		USE nrtype
		REAL(SP), DIMENSION(:), INTENT(INOUT) :: y
		REAL(SP), DIMENSION(:), INTENT(IN) :: dydx,yscal
		REAL(SP), INTENT(INOUT) :: x
		...
		INTERFACE
			...
			REAL(SP), DIMENSION(:), INTENT(OUT) :: dydx
			END SUBROUTINE derivs
	! modified by JMR 8/11/00 :: added subroutine jacobn
			SUBROUTINE jacobn(x,y,dfdx,dfdy)
			USE nrtype
			IMPLICIT NONE
			REAL(SP), INTENT(IN) :: x
