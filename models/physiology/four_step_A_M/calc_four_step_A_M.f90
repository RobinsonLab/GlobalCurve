...
! local variables
	...
	INTERFACE
		SUBROUTINE four_step_derivs(x,y,dydx)
		USE nrtype
		IMPLICIT NONE
		REAL(SP), INTENT(IN) :: x
		...
		END SUBROUTINE
		SUBROUTINE four_step_jacobn(x,y,dfdx,dfdy)
		USE nrtype
		IMPLICIT NONE
		REAL(SP), INTENT(IN) :: x
		REAL(SP), DIMENSION(:), INTENT(IN) :: y
...
!	g_minus=15.0
	...
	do i=1,current_data%len
		...
		call odeint(y,t1,t2,NA_eps,hstart,0.0_sp,four_step_derivs,four_step_jacobn)
		species(i,:)=y(:)
		if (save_steps) then
