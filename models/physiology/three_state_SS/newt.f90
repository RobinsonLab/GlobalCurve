...
!	REAL(SP), PARAMETER :: TOLF=1.0e-4_sp,TOLMIN=1.0e-4_sp,TOLX=epsilon(x),&
		STPMX=100.0
	INTEGER(I4B) :: its
...
! for now just return... if maxits is excedded then we have wacko u,v,w,x,z params
!	call nrerror('MAXITS exceeded in newt')
write(*,*) 'Warning: MAXITS exceeded in newt'
	END SUBROUTINE newt