...
!		call broydn(y,check)
		...
		if (present(g_save)) then
			eta=tsv%nu*(ny(2) + ny(3))
			zeta=tsv%nu*tsv%nu2*ny(3)
			g_save(i)=tsv%g	* (1.+eta*(tsv%w2-1.))**2 * (1.+zeta*(tsv%w-1.))**2
		endif
	end do !i
...
! Begin
			...
			Ca=tsv%Ca
! so (1.0-tsv%alpha) is as normal.
! tsv%alpha is activated.
			sumy=(y(1)+y(2)+y3)/(1.0-tsv%alpha)
			ny(1)=y(1)/sumy
			ny(2)=y(2)/sumy
			ny(3)=y3/sumy+tsv%alpha
!			ny(3)=ny(3)+tsv%alpha
			eta=tsv%nu*(ny(2) + ny(3))
			zeta=tsv%nu*tsv%nu2*ny(3)
			koff=tsv%koff * (1.+eta*(tsv%u-1.))**2 * (1.+zeta*(tsv%x-1.))**2
			f_not=tsv%f*(1.0-tsv%alpha)
			f=f_not * (1.+eta*(tsv%v-1.))**2 * (1.+zeta*(tsv%z-1.))**2
			g=tsv%g	* (1.+eta*(tsv%w2-1.))**2 * (1.+zeta*(tsv%w-1.))**2
!koff=tsv%koff
!f=tsv%f
!g=tsv%g
