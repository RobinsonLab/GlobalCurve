...
! so Fo needs to scale this value to the data.
	...
	tsv%nu=	param_list(current_model%model%param(3)%param_basis(1))%val
	A_M=three_state_SS(current_model,current_data,g_save)
	! A_M contains the n species concentrations.
	ATPase(:)=g_save(:)*A_M(:,3)/tsv%nu
! here we divide by tsv%nu because the ATPase is calculated
! for an assumed head density.  We must reduce this number
! because the functional density is lower than the assumed
! density
