...
!	real(sp),parameter :: pCa_init=6.80 ! from Patel et al. JBC 272,6018-6027(1997)
	real(sp),parameter :: pCa_init=8.0
...
! first we need to pre-equilibrate the muscle
	pCa=pCa_init
	tsv%Ca=10.**-pCa
	tsv%fixed=.true.
...
! the snap sends all the force generating x-bridges into state II.
