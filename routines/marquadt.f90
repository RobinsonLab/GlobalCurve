...
! Global Var's
	...
	!	integer :: global_DOF
	!	integer :: num_expt
	!	real,dimension(num_expt),intent(in) :: local_reduced_chi_sqr 
...
!	global_DOF=num_points-w_num_free_global_param-1
	...
	do i=1,num_free_global_param
		...
		if (param_list(map_from(i))%val>=param_list(map_from(i))%max*(1.0-default%eps) ) then
			write(*,*)'marquadt called with param ',trim(param_list(map_from(i))%name), &
				' out of bounds'
			write(*,*)'setting param to its maximum allowable value'
			param_list(map_from(i))%val=param_list(map_from(i))%max*(1.0-10*default%eps)
		endif
	enddo !i
...
!endif
	...
	do i=1,num_free_global_param
		if ( param_list(map_from(i))%val<= &
			param_list(map_from(i))%min*(1.0+9*default%eps) ) then
...
!			endif
		...
		if ( param_list(map_from(i))%val>= &
			param_list(map_from(i))%max*(1.0-9*default%eps) ) then
			! for now you get no warnings.  If you are less than the min then that's it.
			! you get set to min and no longer count in the chi sqr minimization.
			! A gentler approach would be to give one warning then push the parameter out;
			! then if the param goes out of bounds again then fix it to zero and carry on
			...
			write(*,*)'Maximum exceeded for param: ',trim(param_list(map_from(i))%name), &
				param_list(map_from(i))%val
			param_list(map_from(i))%val=param_list(map_from(i))%max*(1.0-10*default%eps)
			out_of_bounds(i)=out_of_bounds(i)+1
