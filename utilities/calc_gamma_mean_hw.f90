...
! Begin
	...
	gp%R0=R0
	call qdag(gamma_dist,rmin,rmax,errabs,errel,irule,norm,errest)
	call qdag(gamma_mean_TE,rmin,rmax,errabs,errel,irule,res,errest)
	term=res/norm
	TE(1)=term/(1.+term)
	call qdag(calc_1_mom_gamma,rmin,rmax,errabs,errel,irule,res,errest)
	mean(1)=res/norm
	gp%mean=mean(1)
	call qdag(calc_2_mom_gamma,rmin,rmax,errabs,errel,irule,res,errest)
	var=res/norm
